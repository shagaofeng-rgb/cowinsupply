...
async function runNewsAutomationUnlocked({ trigger, dryRun }) {
  ...
  const requestedDrafts = config.dailyTarget;
  ...
  if (!sources.length) {
    await appendNewsPublicationAudit({
      ...
      missingCount: 0,
      status: "blocked",
      message: "NEWS_SOURCE_FEEDS is not configured; no draft was collected and no article was published."
    });
    return finishJob({
      trigger,
      startedAt,
      status: "success",
      metadata: { currentCount, requestedDrafts, requiredEnv: "NEWS_SOURCE_FEEDS", publicPublished: 0 }
    });
  }
  ...
  for (const candidate of candidates) {
    ...
    if (selected.length >= requestedDrafts) break;
  }
  ...
  if (!dryRun) {
    for (const article of selected) {
      ...
      collectedDrafts.push(saved);
      existingNews.push(saved);
      await appendAuditLog({ actor: "news-automation", action: "collect_draft", module: "news", target: saved.slug, result: "pending_review" });
    }
  }
  ...
  await appendNewsPublicationAudit({
    ...
    message: `${collectedDrafts.length} source-backed News drafts collected for editorial review. No article was publicly published by automation.`
  });
  ...
  return finishJob({
    ...
    metadata: {
      ...
      collectedDrafts: collectedDrafts.length,
      publicPublished: 0,
      dryRun
    }
  });
