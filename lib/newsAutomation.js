...
import { sendNewsShortfallAlert } from "@/lib/emailService";
...
async function runNewsAutomationUnlocked({ trigger, dryRun }) {
  ...
  if (!sources.length) {
    const priorShortfall = await hasShortfallAlertForDate(today);
    await appendNewsPublicationAudit({
      date: today,
      timezone: config.timezone,
      targetCount: config.dailyTarget,
    ...
    const alert = priorShortfall ? { sent: false, reason: "already-alerted-today" } : await safeSendShortfallAlert({ date: today, targetCount: config.dailyTarget, publishedCount: currentCount, missingCount: missing, trigger });
    return finishJob({
      trigger,
      startedAt,
      status: "failed",
      errorMessage: "No News RSS sources configured.",
      metadata: { currentCount, missing, requiredEnv: "NEWS_SOURCE_FEEDS", alert }
    });
  }
  ...
  const priorShortfall = finalCount < config.dailyTarget && (await hasShortfallAlertForDate(today));
  await appendNewsPublicationAudit({
    date: today,
    timezone: config.timezone,
    targetCount: config.dailyTarget,
  ...
  const alert = finalCount >= config.dailyTarget || priorShortfall
    ? { sent: false, reason: finalCount >= config.dailyTarget ? "daily-target-met" : "already-alerted-today" }
    : await safeSendShortfallAlert({
        date: today,
        targetCount: config.dailyTarget,
        publishedCount: finalCount,
        missingCount: Math.max(0, config.dailyTarget - finalCount),
  ...
  return [
    {
      ...
      publisher: {
        "@type": "Organization",
        name: "Cowin Supply",
        logo: { "@type": "ImageObject", url: `${SITE_URL}/cowin-assets/cowin-logo.png` }
      },
...
function prepareCandidate(candidate, products, existingNews, config) {
  ...
  const relatedProducts = scoreProducts(candidate, products)
    .filter((item) => item.qualified && item.relevanceScore >= config.relevanceThreshold)
    .slice(0, 3);
  ...
  return {
    ...
    article: {
      ...
      seoIndexable: false,
      contentOrigin: "external_source_draft",
      relevanceStatus: "pending_review",
...
function buildArticleContent(candidate, relatedProducts) {
  ...
  return [
    ...
    `<p>Related Cowin Supply products include ${productLinks}. These links reflect the application context evaluated by Cowin Supply; the original source does not claim that any specific product was used. Customers can review product pages or contact our team for application-specific sourcing support.</p>`,
    "<h2>Source note</h2>",
...
function scoreProducts(candidate, products) {
  ...
  const sourceTopics = sourceApplicationTopics(text);
  return products
    .map((product) => {
      const terms = productSignalTerms(product);
      const matchedTerms = terms.filter((term) => text.includes(term));
      const applicationTopics = productApplicationTopics(product).filter((topic) => sourceTopics.includes(topic));
      const directScore = matchedTerms.reduce((score, term) => score + (term.includes(" ") || term.length > 8 ? 18 : 12), 0);
      const contextScore = applicationTopics.length ? 18 + Math.min(applicationTopics.length - 1, 2) * 3 : 0;
      const qualified = Boolean(directScore || contextScore);
      return {
        ...
        relevanceScore: Math.max(directScore, contextScore),
        matchedTerms,
        applicationTopics,
        qualified,
        relationshipReason: matchedTerms.length
          ? `Matched verified product terms: ${matchedTerms.join(", ")}.`
          : applicationTopics.length
            ? `Matched verified application context: ${applicationTopics.join(", ")}. The source is used for B2B application analysis, not as evidence of product use.`
            : "No verified product or application-context signal was present in the source item.",
        displayOrder: 0
      };
    })
...
function productSignalTerms(product) {
  ...
  const validShortToolTerms = new Set(["saw", "drill", "jig"]);
  const words = raw
    .split(/[^a-z0-9]+/)
    .filter((word) => (word.length >= 4 || validShortToolTerms.has(word)) && !genericTerms.has(word));
  return [...new Set([...phrases, ...words])].sort((a, b) => b.length - a.length).slice(0, 24);
}

function sourceApplicationTopics(text) {
  const topicRules = {
    construction: ["construction", "contractor", "jobsite", "building", "infrastructure", "bim", "mass timber", "bridge", "project"],
    manufacturing: ["manufacturing", "factory", "plant", "fabrication", "workshop", "supply chain"],
    maintenance: ["repair", "maintenance", "right to repair", "service"],
    electrical: ["electrical", "electrician", "wiring", "power distribution"],
    measurement: ["bim", "asset visibility", "layout", "survey", "measurement"]
  };
...
function productApplicationTopics(product) {
  const text = `${product.title || ""} ${product.summary || ""}`.toLowerCase();
  const topics = new Set();
  if (/laser|measur|tape/.test(text)) topics.add("measurement");
  if (/wall|slot|chaser/.test(text)) {
    topics.add("construction");
    topics.add("electrical");
  }
  if (/grinder|cutting|saw|jig/.test(text)) {
    topics.add("construction");
    topics.add("manufacturing");
  }
  if (/drill|screwdriver/.test(text)) {
    topics.add("construction");
    topics.add("manufacturing");
...
async function safeFetch(url) {
  ...
  if (isBlockedSourceHost(parsed.hostname)) throw new Error("Blocked private source host");
...
function isBlockedSourceHost(hostname) {
  ...
  if (/^(127\.|10\.|192\.168\.|169\.254\.|100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\.|172\.(1[6-9]|2\d|3[0-1])\.)/.test(host)) return true;
...
async function hasShortfallAlertForDate(date) {
  const audits = await getNewsPublicationAudits();
  return audits.some((audit) => audit.date === date && ["shortfall", "blocked"].includes(audit.status));
}

async function safeSendShortfallAlert(input) {
  try {
    return await sendNewsShortfallAlert(input);
  } catch (error) {
    console.error("News shortfall alert failed", { message: error instanceof Error ? error.message : "Unknown error" });
