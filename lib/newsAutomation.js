...
export function getNewsAutomationConfig() {
  return {
    ...
    relevanceThreshold: numberEnv("NEWS_RELEVANCE_THRESHOLD", 18),
  ...
  return [
    {
      ...
      mentions: products.map((item) => ({ "@type": "Product", name: item.title, url: `${SITE_URL}/product/${item.slug}.html` }))
    },
    {
      "@context": "https://schema.org",
      "@type": "BreadcrumbList",
...
function prepareCandidate(candidate, products, existingNews, config) {
  ...
  const relatedProducts = scoreProducts(candidate, products).filter((item) => item.relevanceScore >= config.relevanceThreshold).slice(0, 3);
...
function scoreProducts(candidate, products) {
  ...
  return products
    .map((product) => {
      const terms = [product.title, product.category, product.summary, ...(product.tags || [])].filter(Boolean);
      let score = 0;
      for (const term of terms) {
        const chunks = String(term).toLowerCase().split(/[^a-z0-9]+/).filter((word) => word.length > 2);
        for (const chunk of chunks) {
          if (text.includes(chunk)) score += chunk.length > 6 ? 8 : 4;
        }
      }
      return {
        ...
        relationshipReason: `Matched source topic against ${product.title} category and description.`,
        displayOrder: 0
      };
    })
