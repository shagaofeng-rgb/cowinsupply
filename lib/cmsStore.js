...
export async function getCmsItems(type, { includeInactive = false } = {}) {
  ...
  let items = materialized.filter((item) => item.type === type).map(normalizeCmsItemText);
...
function normalizeCmsItemText(value) {
  if (typeof value === "string") return normalizeCompanyText(value);
...
export async function getInquiryDetail(id) {
  ...
  return buildInquiryDetail(inquiry, await getVisitEvents());
}
...
export async function getInquiryDetails(ids = []) {
  ...
  return new Map(inquiries.filter((item) => requested.has(item.id)).map((item) => [item.id, buildInquiryDetail(item, events)]));
}

function buildInquiryDetail(inquiry, events) {
  const visitorId = String(inquiry.visitorId || "");
  const sessionId = String(inquiry.sessionId || "");
  const journey = events
    .filter((event) => (visitorId && event.visitorId === visitorId) || (sessionId && event.sessionId === sessionId))
    .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));

  return {
    inquiry,
    journey,
    journeyAvailable: Boolean(visitorId || sessionId),
    summary: {
