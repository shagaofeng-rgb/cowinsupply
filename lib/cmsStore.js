...
import { getLiveNewsItems } from "@/lib/liveNewsFeed";
...
export async function getCmsItems(type, { includeInactive = false } = {}) {
  ...
  if (type === "news") {
    const products = visibleItems(merged.filter((item) => item.type === "product"), false);
    const liveNews = await getLiveNewsItems(products);
    const existingSlugs = new Set(items.map((item) => item.slug));
    items = [...liveNews.filter((item) => !existingSlugs.has(item.slug)), ...items];
  }
...
export async function getSystemStatus() {
  ...
  return {
    ...
    dataStore: process.env.VERCEL ? "Vercel 临时文件存储" : "本地 .data 文件存储",
