...
async function buildInquiryDetail(inquiry) {
  ...
  const [journey, activities, notifications] = await Promise.all([
    hasPersistentStore()
      ? listPersistentVisitEvents({ visitorId, sessionId, limit: 500 })
      : (await getVisitEvents()).filter((event) => (visitorId && event.visitorId === visitorId) || (sessionId && event.sessionId === sessionId)),
