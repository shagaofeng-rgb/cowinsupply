  ...
  const normalized = {
    ...
    ...(input.type === "product"
      ? {
          ...
          legacySourceUrl: input.legacySourceUrl || "",
          legacyDescriptionHtml: input.legacyDescriptionHtml || "",
          publishedAt: input.publishedAt || (input.status === "published" ? now : ""),
          seoTitle: input.seoTitle || input.title,
          seoDescription: input.seoDescription || input.summary,
          canonicalUrl: input.canonicalUrl || `https://www.cowinsupply.com/product/${slug}.html`,
          ...
          seoIndexable: input.seoIndexable ?? (Array.isArray(input.specifications) && input.specifications.some((row) => row?.label && row?.value))
        }
      : input.type === "news"
      ? {
...
function parseNewsSourcesFromEnv() {
  const whitelist = String(process.env.NEWS_SOURCE_WHITELIST || "")
    ...
    .map((item) => item.trim())
    .filter(Boolean);
  const feeds = String(process.env.NEWS_SOURCE_FEEDS || DEFAULT_NEWS_FEEDS.join(","))
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
