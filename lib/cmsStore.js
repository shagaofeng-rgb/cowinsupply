...
const DEFAULT_SITE_SETTINGS = {
  ...
  companyEmail: "info@cowinsupply.com",
...
export async function getCmsItems(type, { includeInactive = false } = {}) {
  ...
  let items = materialized.filter((item) => item.type === type).map(normalizeCmsItemText).map(item => normalizeNewsPresentation(item, materialized));
...
function normalizeNewsPresentation(item, catalog) {
  if (item?.type !== "news" || !item.title) return item;
  const title = String(item.title);
  const normalizedTitle = title ? `${title[0].toUpperCase()}${title.slice(1)}` : title;
  // Product buyer briefings must show the referenced model, not a generic fallback tool.
  const productSlug = item.relatedProducts?.[0]?.productSlug;
  const product = String(item.slug || '').includes('buyer-briefing') && productSlug
    ? catalog.find(entry => entry.type === 'product' && entry.slug === productSlug && !['offline', 'draft', 'deleted'].includes(entry.status))
    : null;
  return {
    ...item,
    ...(product?.image ? { image: product.image, coverImageAlt: product.title } : {}),
...
export async function recordInquiryNotification(input) {
  const notification = {
    ...
    provider: "smtp",
    recipient: String(input.recipient || companyProfile.email),
...
export async function getSiteSettings() {
  return {
    ...
    adminNotificationEmail: companyProfile.email
  };
}
