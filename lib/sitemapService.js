...
import { PRODUCT_TAXONOMY } from "@/lib/catalogTaxonomy";
...
async function getSitemapSnapshot() {
  ...
  if (cached?.version === 6 && cached?.xml?.pages && cached?.xml?.newsContent && cached?.xml?.news && cached?.generatedAt) return cached;
...
async function buildSitemapSnapshot() {
  ...
  const buckets = {
    pages: [...staticPages],
    products: products.filter(isPublishedProduct).map((item) => ({
  ...
  return {
    // Version 6 adds a standard, full News sitemap alongside the time-limited
    // Google News sitemap, while retaining prior noindex exclusions.
    version: 6,
