...
async function getSitemapSnapshot() {
  ...
  if (cached?.version === 3 && cached?.xml?.pages && cached?.xml?.news && cached?.generatedAt) return cached;
...
async function buildSitemapSnapshot() {
  ...
  const buckets = {
    pages: [...staticPages],
    products: products.filter(isIndexableProduct).map((item) => ({
...
function categoryUrls() {
  return ["wall-chasers", "brushless-angle-grinders", "cordless-power-tools", "brushless-saws", "laser-measuring-tools", "specialty-tools"].map((slug) => ({
...
function isIndexableProduct(item) {
  return item?.status === "published" && item?.seoIndexable !== false && Boolean(item?.title && item?.summary && (item?.specifications || []).length);
}
