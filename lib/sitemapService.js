...
async function getSitemapSnapshot() {
  ...
  if (cached?.version === 2 && cached?.xml?.pages && cached?.xml?.news && cached?.generatedAt) return cached;
  return buildSitemapSnapshot();
}

async function buildSitemapSnapshot() {
  const [products, news, blog, staticPages, tagPages] = await Promise.all([
    getCmsItems("product"),
    getCmsItems("news"),
    getCmsItems("blog"),
    getStaticPages(),
