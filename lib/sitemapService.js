...
import { getCategorySummary, getCmsItems } from "@/lib/cmsStore";
...
async function buildSitemapSnapshot() {
  const [products, news, categories, staticPages, tagPages] = await Promise.all([
    getCmsItems("product"),
    getCmsItems("news"),
    getCategorySummary(),
    getStaticPages(),
  ...
  const buckets = {
    ...
    categories: [
      ...
      ...categories
        ...
        .map((item) => ({
          loc: `${SITE_URL}/tag/${safePathSegment(item.name)}/`,
          lastmod: today(),
          source: "cms-category"
        }))
    ]
