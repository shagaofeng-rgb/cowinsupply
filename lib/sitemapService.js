...
async function getSitemapSnapshot() {
  ...
  if (cached?.version === 5 && cached?.xml?.pages && cached?.xml?.news && cached?.generatedAt) return cached;
...
async function buildSitemapSnapshot() {
  ...
  return {
    // Version 5 invalidates the prior snapshot, which incorrectly included
    // noindex product URLs and the post-inquiry confirmation page.
    version: 5,
