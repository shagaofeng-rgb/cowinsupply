...
import { isCanonicalProductRecord, PRODUCT_TAXONOMY } from "@/lib/catalogTaxonomy";
...
const STATIC_PAGES = [
  ...
  { path: "/blog", file: "index.html", group: "pages" },
...
const GROUPS = [
  ...
  // The regular sitemap keeps every eligible News URL discoverable. The
  // Google News-specific sitemap is intentionally limited to recent items.
  { key: "newsContent", filename: "sitemap-news.xml" },
  { key: "news", filename: "news-sitemap.xml" },
...
async function getSitemapSnapshot() {
  ...
  if (cached?.version === 7 && cached?.xml?.pages && cached?.xml?.newsContent && cached?.xml?.news && cached?.generatedAt) return cached;
  return buildSitemapSnapshot();
}

async function buildSitemapSnapshot() {
  const [products, news, blog, staticPages] = await Promise.all([
    getCmsItems("product"),
    getCmsItems("news"),
    getCmsItems("blog"),
    getStaticPages()
  ...
  const buckets = {
    pages: [...staticPages],
    products: products.filter((item) => isCanonicalProductRecord(item) && isPublishedProduct(item)).map((item) => ({
      loc: `${SITE_URL}/product/${safePathSegment(item.slug)}.html`,
      lastmod: toDate(item.updatedAt || item.publishedAt || item.createdAt),
      source: "cms-product"
    })),
    blog: blog.filter(isIndexableEditorialItem).map((item) => ({
        loc: `${SITE_URL}/blog/${safePathSegment(item.slug)}`,
        lastmod: toDate(item.updatedAt || item.publishedAt || item.createdAt),
        source: "cms-blog"
      })),
    newsContent: news.filter(isIndexableEditorialItem)
      .map((item) => ({
        loc: `${SITE_URL}/news/${safePathSegment(item.slug)}`,
        lastmod: toDate(item.updatedAt || item.publishedAt || item.createdAt),
        source: "cms-news"
      })),
    news: news.filter((item) => isIndexableEditorialItem(item) && isRecentNews(item))
      .map((item) => ({
        loc: `${SITE_URL}/news/${safePathSegment(item.slug)}`,
        lastmod: toDate(item.updatedAt || item.publishedAt || item.createdAt),
        publicationDate: item.publishedAt || item.createdAt,
        title: item.title,
        language: item.language || item.sourceLanguage || "en",
        source: "cms-news"
      })),
  ...
  const groups = GROUPS.map((group) => {
    ...
    xml[group.key] = group.key === "news" ? buildNewsUrlset(urls) : buildUrlset(urls);
    return {
      ...group,
  ...
  return {
    // Version 7 preserves the full News sitemap and stops changing category
    // lastmod every day when no category content changed.
    version: 7,
...
function categoryUrls() {
  return PRODUCT_TAXONOMY.map((category) => ({
    loc: `${SITE_URL}/products/${category.slug}`,
    // Category pages are taxonomy records, not newly edited every calendar day.
    // A stable, truthful date avoids emitting a misleading recrawl signal.
    lastmod: CONTENT_BASELINE_DATE,
...
function buildNewsUrlset(urls) {
  const rows = urls
    .map((url) => `  <url>
    <loc>${escapeXml(url.loc)}</loc>
    <news:news>
      <news:publication><news:name>Cowin Supply</news:name><news:language>${escapeXml(newsLanguage(url.language))}</news:language></news:publication>
      <news:publication_date>${escapeXml(new Date(url.publicationDate).toISOString())}</news:publication_date>
...
function isRecentNews(item) {
  ...
  return Number.isFinite(publishedAt) && publishedAt >= Date.now() - 48 * 60 * 60 * 1000;
}

function isIndexableEditorialItem(item) {
  return item?.status === "published" && item?.seoIndexable !== false && Boolean(item?.title && (item?.content || item?.summary));
}

function isPublishedProduct(item) {
  // A sitemap must only advertise pages that are eligible for indexing. Historical
  // catalogue records without verified specifications intentionally render noindex.
  return item?.status === "published" && item?.seoIndexable !== false && Boolean(item?.title && item?.summary);
}

function newsLanguage(value) {
