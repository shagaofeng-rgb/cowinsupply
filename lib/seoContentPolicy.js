...
export const SEO_CLEANUP_VERSION = "2026-08-product-data-foundation-v4";
...
function normalizeProductRecords(items) {
  return items.map((item) => {
    ...
    const specifications = Array.isArray(item.specifications) ? item.specifications.filter((row) => row?.label && row?.value && row?.verified === true) : [];
    const currentFeatures = Array.isArray(item.features) ? item.features : [];
    return {
      ...normalized,
      specifications,
      // Records without a verified specification set remain enquiry-visible but are not indexable.
      ...
      geoSummary: normalized.geoSummary
    };
  });
}
