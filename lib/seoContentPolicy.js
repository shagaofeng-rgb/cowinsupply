export const SEO_CLEANUP_VERSION = "2026-08-content-quality-v3";
...
function normalizeProductRecords(items) {
  return items.map((item) => {
    ...
    const specifications = Array.isArray(item.specifications) ? item.specifications.filter((row) => row?.label && row?.value) : [];
    const currentFeatures = Array.isArray(item.features) ? item.features : [];
    return {
      ...item,
      category: productCategory(item),
      specifications,
      // Product records with no confirmed spec row remain enquiry-visible but cannot be indexed.
      seoIndexable: specifications.length > 0,
      features: currentFeatures.filter((feature) => !/legacy catalog|previous owned website|technical confirmation available/i.test(String(feature))),
      geoSummary: safeProductSummary(item)
    };
  });
}
...
function productCategory(item) {
  ...
  if (/wall chas|wall slot|wall groov|concrete wall cut/.test(text)) return "Wall Chasers";
  if (/jig saw|cold cut|annular cutter|\bsaw\b/.test(text)) return "Brushless Saws";
  if (/angle grinder|\bgrinder\b/.test(text)) return "Brushless Angle Grinders";
  if (/laser|distance meter|measuring tape|diastimeter/.test(text)) return "Laser Measuring Tools";
  if (/drill|screwdriver/.test(text)) return "Cordless Power Tools";
  return "Specialty Tools";
}

function safeProductSummary(item) {
  const title = String(item.title || "Product").replace(/,?\s*Ltd\.?$/i, "").trim();
  return `${title}. Contact Cowin Supply with your application, voltage and quantity to confirm the current configuration and technical details.`;
}
