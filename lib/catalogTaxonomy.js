...
// Historical catalog URLs that represent the same sellable model as a current
// canonical record. They remain in CMS history but must not be advertised.
export const LEGACY_PRODUCT_REDIRECTS = new Map([
  ["40", "CuttingMachine"]
]);

export function canonicalProductSlug(slug) {
  return LEGACY_PRODUCT_REDIRECTS.get(String(slug || "")) || String(slug || "");
}

export function isCanonicalProductRecord(item) {
  return canonicalProductSlug(item?.slug) === String(item?.slug || "");
}
