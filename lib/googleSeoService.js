...
export async function fetchGoogleSeoData({ siteUrl = process.env.GSC_SITE_URL || "sc-domain:cowinsupply.com", from = "", to = "" } = {}) {
  ...
  try {
    ...
    const dateRange = resolveSearchConsoleRange({ from, to });
    ...
    // A dimension-free query is the only reliable source for the period total.
    // Summing the top query/page rows double-counts the same search activity and
    // silently undercounts anything outside the first rowLimit records.
    const [summaryRows, queryRows, pageRows, countryRows] = await Promise.all([
      searchAnalytics(token, siteUrl, baseBody),
    ...
    const total = summaryRows[0] || {};
    const clicks = Number(total.clicks || 0);
    const impressions = Number(total.impressions || 0);
    const ctr = Number(((total.ctr || 0) * 100).toFixed(2));
    const position = Number((total.position || 0).toFixed(1));
...
export async function inspectUrlInGoogle({ siteUrl = process.env.GSC_SITE_URL || "sc-domain:cowinsupply.com", inspectionUrl } = {}) {
  const account = getServiceAccount();
  if (!account?.client_email || !account?.private_key) {
    return { success: false, skipped: true, inspectionUrl, error: "not-configured" };
  }
  if (!inspectionUrl) {
    return { success: false, skipped: true, inspectionUrl: "", error: "missing-inspection-url" };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return { success: false, skipped: false, inspectionUrl, status: response.status, error: sanitizeError(data.error?.message || `inspection-${response.status}`) };
    }
    ...
    return { success: false, skipped: false, inspectionUrl, error: sanitizeError(error) };
  }
}
...
function resolveSearchConsoleRange({ from, to }) {
  const yesterday = new Date();
  yesterday.setUTCDate(yesterday.getUTCDate() - 1);
  const defaultEnd = yesterday.toISOString().slice(0, 10);
  const defaultStartDate = new Date(`${defaultEnd}T00:00:00Z`);
  defaultStartDate.setUTCDate(defaultStartDate.getUTCDate() - 28);
  const defaultStart = defaultStartDate.toISOString().slice(0, 10);
  const datePattern = /^\d{4}-\d{2}-\d{2}$/;
  const safeFrom = datePattern.test(from) ? from : defaultStart;
  const safeTo = datePattern.test(to) && to <= defaultEnd ? to : defaultEnd;
  return safeFrom <= safeTo ? { startDate: safeFrom, endDate: safeTo } : { startDate: defaultStart, endDate: defaultEnd };
}
