...
function matchProducts(raw, products) {
  ...
  const generic = new Set(["power", "tool", "tools", "industrial", "professional", "machine", "machines", "supply", "heavy", "duty", "and", "with", "for"]);
  return products.map((product) => {
    const words = `${product.title} ${product.category || ""} ${(product.tags || []).join(" ")}`.toLowerCase().split(/[^a-z0-9]+/).filter((word) => word.length >= 4 && !generic.has(word));
    const matched = [...new Set(words.filter((word) => text.includes(word)))];
    return { ...product, matched, score: matched.length };
  }).filter((item) => item.score >= 2 && item.image).sort((a, b) => b.score - a.score).slice(0, 3);
}
...
function stripHtml(value) { return String(value || "").replace(/<!\[CDATA\[|\]\]>/g, "").replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim(); }
