...
import { compactNewsTitle, countCandidateReasons, newsSeoTitle, NEWS_POLICY_VERSION } from "@/lib/newsReliability";
...
const AUTOMATION_VERSIONS = new Set(["v2", "v3", "v4", NEWS_POLICY_VERSION]);
...
const ROTATION_REASONS = new Set(["source_rotation_cooldown", "source_group_consecutive_limit"]);
const FALLBACK_REASONS = new Set([...ROTATION_REASONS, "source_age_fallback_window"]);
...
const DEFAULT_CONFIG = {
  ...
  // URL and title deduplication catch repeat reporting first. This threshold avoids
  // rejecting a new source-backed article merely because shared site copy is similar.
  contentSimilarityThreshold: 0.55,
...
export async function getNewsAutomationDashboard() {
  ...
  const latestIngest = runs.find((item) => item.phase === "ingest") || null;
  const latestPublish = runs.find((item) => item.phase === "publish") || null;
  const scheduled = candidates.filter((item) => item.status === "scheduled");
  const deferredByRotation = scheduled.filter((item) => item.availabilityReasons?.length).length;
  return {
    config,
    state,
    ...
    published: published.filter((item) => AUTOMATION_VERSIONS.has(item.automationVersion)).slice(0, 30),
    metrics: {
      ...
      publishedByV2: published.filter((item) => AUTOMATION_VERSIONS.has(item.automationVersion)).length,
      lastSuccessfulPublishAt: state.lastSuccessfulPublishAt || ""
    },
    health: {
      latestIngest,
      latestPublish,
      readyCandidates: scheduled.filter((item) => !item.availabilityReasons?.length).length,
      deferredByRotation,
      retryPending: candidates.filter((item) => item.status === "retry_pending").length,
      needsReview: candidates.filter((item) => item.status === "needs_review").length,
      nextAction: describeNextNewsAction({ state, scheduled, latestPublish })
    },
...
export async function runNewsDaily({ trigger = "manual", dryRun = false } = {}) {
  ...
  const outcome = { startedAt, completedAt: new Date().toISOString(), trigger, dryRun, ingest, publish };
  console.info("news-automation-run", JSON.stringify({
    trigger,
    dryRun,
    ingest: ingest.status,
    discovered: ingest.discovered || 0,
    candidates: ingest.quality?.candidate || 0,
    activeCandidates: ingest.quality?.activeCandidate || 0,
    rejectionReasons: ingest.quality?.rejectionReasons || {},
    publish: publish.status,
    reason: publish.reason || publish.publishResult?.reason || "",
    published: Boolean(publish.publishResult?.published),
    slug: publish.publishResult?.slug || ""
  }));
  return outcome;
}
...
export async function withdrawNewsArticle(slug, actor = "admin") {
  ...
  const item = items.find((entry) => entry.slug === slug && AUTOMATION_VERSIONS.has(entry.automationVersion));
...
async function runIngestUnlocked({ trigger, dryRun }) {
  ...
  const publishedNews = existingNews.filter((item) => item.status === "published");
  const sourceResults = [];
  const discovered = [];
  ...
  for (const raw of discovered) {
    const existingIndex = nextCandidates.findIndex((item) => item.sourceFingerprint === raw.sourceFingerprint);
    const existing = existingIndex >= 0 ? nextCandidates[existingIndex] : null;
    const candidate = prepareIngestCandidate(raw, {
      products,
      existingNews: publishedNews,
      candidates: existing ? nextCandidates.filter((item) => item.id !== existing.id) : nextCandidates,
      config
    });
    if (existing) {
      if (canReassessCandidate(existing)) {
        nextCandidates[existingIndex] = {
          ...candidate,
          id: existing.id,
          discoveredAt: existing.discoveredAt || candidate.discoveredAt,
          reassessedAt: new Date().toISOString(),
          reassessmentReason: "publication_template_updated"
        };
        created.push(nextCandidates[existingIndex]);
        reassessed.push(nextCandidates[existingIndex]);
      }
      continue;
    }
  ...
  const activePool = nextCandidates.filter((item) => !["archived", "published_success"].includes(item.status));
  return recordRun({
    ...
    quality: {
      candidate: created.filter((item) => item.status === "candidate").length,
      rejected: created.filter((item) => item.status === "rejected").length,
      activeCandidate: activePool.filter((item) => ["candidate", "scheduled", "retry_pending"].includes(item.status)).length,
      rejectionReasons: countCandidateReasons(activePool)
    }
  });
}
...
async function runPublicationUnlocked({ trigger, dryRun }) {
  ...
  const publishedNews = existingNews.filter((item) => item.status === "published");
  const candidates = hydratePublicationCandidates(savedCandidates, { products, existingNews: publishedNews, config });
  ...
  const primaryCandidate = pendingCandidate || nextCandidates.find((item) => canPublish(state, item));
  // A verified candidate remains usable after the normal source rotation window when
  // no other qualified source is available. The fallback is retained in the audit trail.
  const fallbackCandidate = !primaryCandidate
    ? nextCandidates.find((item) => canPublish(state, item, { allowRotationFallback: true }))
    : null;
  const originalFallbackCandidate = !primaryCandidate && !fallbackCandidate
    ? buildOriginalFallbackCandidate({ products, existingNews: publishedNews, state, config })
    : null;
  if (originalFallbackCandidate) nextCandidates.unshift(originalFallbackCandidate);
  const eligible = primaryCandidate || fallbackCandidate || originalFallbackCandidate;
  const selectionMode = originalFallbackCandidate ? "cowinsupply_original_fallback" : fallbackCandidate ? "rotation_or_age_fallback" : "primary";
  let publishResult = { published: false, reason: eligible ? "dry-run" : nextExecutionReason(state, nextCandidates) };
  if (eligible && !dryRun) {
    const publishIndex = nextCandidates.findIndex((item) => item.id === eligible.id);
      nextCandidates[publishIndex] = { ...eligible, status: "publishing", publishingAt: new Date().toISOString(), selectionMode, retryCount: Number(eligible.retryCount || 0) + (eligible.status === "failed" ? 1 : 0) };
    await setValue(CANDIDATES_KEY, nextCandidates.slice(0, MAX_CANDIDATES));
    let saved;
    try {
      const article = buildPublishedArticle(eligible, [...publishedNews, ...products]);
  ...
  if (!dryRun) await setValue(CANDIDATES_KEY, nextCandidates.slice(0, MAX_CANDIDATES));
  const rejectionReasons = countCandidateReasons(nextCandidates.filter((item) => ["needs_review", "rejected"].includes(item.status)));
  return recordRun({
    phase: "publish",
    trigger,
    dryRun,
    status: publishResult.published ? "published" : "failed",
    reason: publishResult.reason,
    startedAt,
    selectionMode,
    quality: {
      scheduled: nextCandidates.filter((item) => item.status === "scheduled").length,
      deferredByRotation: nextCandidates.filter((item) => item.status === "scheduled" && item.availabilityReasons?.length).length,
      rejected: nextCandidates.filter((item) => item.status === "rejected").length,
      rejectionReasons
    },
    publishResult
  });
...
function prepareCandidate(raw, { products, existingNews, candidates, config }) {
  ...
  if (ageMs > config.fallbackCandidateMaxAgeDays * 86400000) reasons.push("source_older_than_fallback_window_or_missing_date");
  else if (ageMs > config.candidateMaxAgeHours * 3600000) reasons.push("source_age_fallback_window");
  ...
  const availabilityReasons = reasons.filter((reason) => FALLBACK_REASONS.has(reason));
  const blockingReasons = publicationBlockers(reasons.filter((reason) => !FALLBACK_REASONS.has(reason)));
  const warnings = reasons.filter((reason) => !blockingReasons.includes(reason) && !availabilityReasons.includes(reason));
  const status = blockingReasons.length ? "needs_review" : "scheduled";
  return {
    id, siteId: SITE.siteId, policyVersion: NEWS_POLICY_VERSION, status, discoveredAt: now, fetchedAt: raw.sourceFetchedAt, verifiedAt: now, plannedAt: now, generatedAt: now, qualityCheckedAt: now,
    ...
    article, humanizerAudit, rejectionReasons: blockingReasons, availabilityReasons, qualityWarnings: warnings,
...
function prepareIngestCandidate(raw, { products, existingNews, candidates, config }) {
  ...
  const availabilityReasons = reasons.filter((reason) => FALLBACK_REASONS.has(reason));
  const qualityReasons = reasons.filter((reason) => !FALLBACK_REASONS.has(reason));
  const score = Math.max(0, 100 - qualityReasons.reduce((total, reason) => total + (reason.includes("duplicate") ? 45 : reason.includes("source") ? 30 : 15), 0));
  const accepted = qualityReasons.length === 0 && score >= config.minScore;
  return {
    id: crypto.randomUUID(), siteId: SITE.siteId, policyVersion: NEWS_POLICY_VERSION, status: accepted ? "candidate" : "rejected", discoveredAt: now, normalizedAt: now, verifiedAt: now, scoredAt: now,
    sourceUrl: raw.sourceUrl, sourceFingerprint: raw.sourceFingerprint, eventFingerprint: raw.eventFingerprint, source: raw.source,
    sourceTitle: raw.title, sourceSummary: raw.summary, sourceAuthor: raw.author, sourcePublishedAt: raw.sourcePublishedAt, sourceFetchedAt: raw.sourceFetchedAt,
    ...
    rejectionReasons: qualityReasons, availabilityReasons, qualityWarnings: [], article: null,
    qualityGate: { source: !qualityReasons.some((reason) => reason.includes("source")), relevance: Boolean(related.length), duplicate: !qualityReasons.some((reason) => reason.includes("duplicate")), image: Boolean(related[0]?.image), seo: false }
  };
}
...
function hydratePublicationCandidates(candidates, { products, existingNews, config }) {
  return candidates.map((candidate) => {
    ...
    if (candidate.contentKind === "original_editorial" && candidate.article) return candidate;
    const raw = {
    ...
    if (rebuilt.status !== "scheduled") return { ...candidate, status: "rejected", rejectionReasons: rebuilt.rejectionReasons, availabilityReasons: rebuilt.availabilityReasons || [], rejectedAt: new Date().toISOString() };
    return {
      ...candidate,
      ...rebuilt,
      id: candidate.id,
...
function canReassessCandidate(candidate) {
  if (candidate?.policyVersion !== NEWS_POLICY_VERSION) return candidate?.status === "rejected";
  const reasons = candidate?.rejectionReasons || [];
  const qualityTemplateFailure = reasons.length === 1 && [
    "content_below_required_word_count",
    "content_above_required_word_count",
    "humanizer_quality_gate_failed"
  ].includes(reasons[0]);
  const failedPublicationArtifact = Number(candidate?.retryCount || 0) > 0 && reasons.length > 0 && reasons.every((reason) => [
    "duplicate_source_url",
    "duplicate_or_similar_title",
    "semantic_content_similarity_threshold"
  ].includes(reason));
  return candidate?.status === "rejected" && (qualityTemplateFailure || failedPublicationArtifact);
}

function buildOriginalFallbackCandidate({ products, existingNews, state, config }) {
  if (!isExecutionDue(state, config)) return null;
  const eligibleProducts = products.filter((product) => product?.image && canUseProductForAutomatedNews(product));
  if (!eligibleProducts.length) return null;
  const day = shanghaiDay();
  const recentSlugs = existingNews
    .flatMap((item) => item.relatedProducts || [])
    .map((item) => item.productSlug)
    .filter(Boolean);
  const primary = eligibleProducts.find((item) => !recentSlugs.slice(0, 6).includes(item.slug)) || eligibleProducts[dateSeed(day) % eligibleProducts.length];
  const article = generateOriginalBriefingArticle(primary, day, { products, existingNews });
  const now = new Date().toISOString();
  return {
    id: `original-${day}-${primary.slug}`,
    siteId: SITE.siteId,
    policyVersion: NEWS_POLICY_VERSION,
    status: "scheduled",
    contentKind: "original_editorial",
    discoveredAt: now,
    verifiedAt: now,
    qualityCheckedAt: now,
    sourceUrl: `${SITE_URL}/product/${primary.slug}.html`,
    sourceFingerprint: sha256(`cowinsupply-original|${day}|${primary.slug}`),
    eventFingerprint: sha256(`buyer-briefing|${day}|${primary.category}`),
    sourceTitle: article.title,
    sourceSummary: article.summary,
    sourceAuthor: "Cowin Supply Editorial",
    sourcePublishedAt: now,
    sourceFetchedAt: now,
    source: { id: "cowinsupply-original", publisher: "Cowin Supply", sourceGroup: "Cowin Supply Original", trustLevel: "high" },
    productIds: [primary.id],
    productSlugs: [primary.slug],
    article,
    humanizerAudit: { passed: true, mode: "original_editorial_fallback", processedAt: now },
    rejectionReasons: [],
    availabilityReasons: [],
    qualityWarnings: ["external_sources_had_no_publishable_candidate"],
    qualityGate: { source: true, relevance: true, duplicate: true, image: true, humanizer: true, wordCount: article.wordCount, seo: true }
  };
}

function generateOriginalBriefingArticle(primary, day, { products = [], existingNews = [] } = {}) {
  const scenario = inferScenario(primary);
  const category = primary.category || "professional power tools";
  const title = compactNewsTitle(`${primary.title} Buyer Briefing`);
  const productUrl = `/product/${primary.slug}.html`;
  const categoryUrl = categoryUrlFor(primary);
  const cover = selectNewsCoverImage({ raw: { sourceFingerprint: sha256(`${day}|${primary.slug}`) }, primary, related: [primary], products, existingNews });
  const content = `<h2>Purpose of this buyer briefing</h2><p>This Cowin Supply editorial briefing gives distributors, contractors and procurement teams a structured way to review ${escapeHtml(primary.title)} for ${escapeHtml(scenario)}. It is based on the current Cowin Supply catalog record and general procurement practice. It does not report an external event, promise a particular result or replace the product specification, jobsite method statement or local safety requirements.</p>
<h2>Start with the work requirement</h2><p>Reliable tool selection begins with the finished task. The buyer should record the material, dimensions, access conditions, expected duty cycle and finish requirement before comparing machines. A product category can indicate the kind of work a tool supports, but the final configuration depends on the real material and operating environment. This distinction is especially important for distributors preparing quotations for customers whose site conditions may differ from the original enquiry.</p><p>The review should identify whether the work takes place in a new building, an occupied interior, a workshop or an exposed construction area. Dust, moisture, ventilation, working height and nearby services can change the required controls. Recording these conditions early helps the supplier discuss the correct voltage, plug, consumable and accessory arrangement without treating a catalog title as a complete technical specification.</p>
<h2>Product context</h2><p><a href="${productUrl}">${escapeHtml(primary.title)}</a> appears in the Cowin Supply catalog under ${escapeHtml(category)}. Its published description should be checked together with the available images and technical data. For ${escapeHtml(scenario)}, buyers should confirm the material range, working capacity, compatible consumables and intended operating pattern. If any required value is not present in the published record, it should be requested before an order is finalized.</p><p>${escapeHtml(productReviewGuidance(primary))}</p>
<h2>Questions for a useful quotation</h2><p>A useful quotation request includes the destination market, quantity, voltage, plug type, application material and expected operating schedule. Buyers should also state whether they need samples, private labeling, packaging changes, spare consumables or supporting documents. These details allow the commercial response to separate confirmed catalog information from options that require further review.</p><p>Distributors can reduce later clarification by describing the customer profile and sales channel. A contractor buying for a defined project may prioritize application fit and delivery timing, while a wholesale program may also need packaging consistency, replacement accessories and repeat-order planning. Neither case should rely on assumptions. The requested configuration and supporting documents should be written into the enquiry and confirmed before production or shipment.</p>
<h2>Power, accessories and consumables</h2><p>Electrical supply and consumable selection are part of the tool configuration, not secondary details. The buyer should confirm the available voltage and frequency, the required plug, extension arrangements where applicable, and any limits imposed by the work area. Blades, bits, abrasives and measuring accessories must be selected for the material and task. A compatible size alone does not establish suitability; the responsible operator should follow the applicable product instructions and local procedures.</p><p>For repeat purchasing, distributors should consider how wear items will be identified and replenished. A quotation can include the initial machine quantity together with a realistic consumable plan, but the plan should be based on the expected material and workload. Keeping machine and consumable references together also makes after-sales communication clearer when a customer asks for replacements.</p>
<h2>Jobsite controls and operator responsibility</h2><p>Professional equipment still depends on trained operation, inspection and appropriate personal protective equipment. Before use, the responsible team should confirm guarding, tool condition, workpiece support, dust or debris control and the safe operating zone. Existing utilities, reinforcement, hidden services and surrounding finishes may require additional checks before cutting, drilling, slotting, grinding or measuring begins.</p><p>Procurement documentation should avoid turning a general product description into a safety approval or performance guarantee. The catalog helps start the technical discussion. The final work method remains the responsibility of the buyer, contractor and trained operator working under the rules that apply at the destination.</p>
<h2>Inspection, maintenance and handover</h2><p>A practical maintenance plan starts before the tool enters service. The receiving team can record model and configuration details, inspect packaging and accessories, and establish who is responsible for routine checks. Cleaning, storage and consumable replacement should follow the applicable instructions. Where the tool moves between crews or projects, a basic handover record can help identify damage, missing accessories or changes in operating condition.</p><p>Distributors can support customers by keeping product references, quotation details and follow-up questions linked to the same order record. This makes it easier to distinguish a product issue from an application, consumable or supply mismatch. Clear records also improve future stock planning because repeat demand can be connected to the configuration actually supplied.</p>
<h2>Information to confirm with Cowin Supply</h2><p>Before ordering, confirm the current specification, available configuration, minimum order quantity, packaging, lead time and destination-market requirements. Product images and page descriptions are useful references, but they should not be used to infer an unlisted certification, accessory or performance value. Questions can be sent through the <a href="/contact">Cowin Supply contact page</a> with the material, task, quantity and destination market included.</p><p>Buyers comparing several products should explain the decision they are trying to make. For example, they may be choosing between configurations for different materials, looking for a compact option for restricted access, or preparing a distributor range for several customer groups. A specific question produces a more useful answer than a request for a general price list without application context.</p>
<h2>Next step</h2><p>Review the current <a href="${productUrl}">${escapeHtml(primary.title)}</a> record, compare the <a href="${categoryUrl}">${escapeHtml(category)}</a> category, and prepare a quotation request that states the material, application, quantity, voltage and destination. Cowin Supply can then respond against a defined requirement. This briefing is original Cowin Supply editorial content created to keep buyers informed when no suitable external industry report passes the automated publication checks.</p>`;
  const contentText = stripHtml(content);
  const faq = [
    { question: `What should buyers confirm for ${primary.title}?`, answer: "Confirm the material, task, voltage, plug, quantity, consumables and destination-market requirements before ordering." },
    { question: "Is this article a product performance guarantee?", answer: "No. It is an original procurement briefing; final suitability depends on confirmed specifications and actual working conditions." },
    { question: "Where can a buyer request a quotation?", answer: "Use the Cowin Supply contact page and include the application, material, quantity and destination market." }
  ];
  return {
    title,
    summary: truncate(`An original Cowin Supply buyer briefing for reviewing ${primary.title}, application requirements, configuration details and quotation information.`, 250),
    content,
    contentText,
    wordCount: contentText.split(/\s+/).filter(Boolean).length,
    image: cover.image,
    imageAlt: `${cover.product.title} buyer briefing`,
    seoTitle: newsSeoTitle(title),
    seoDescription: truncate(`Buyer guidance for ${primary.title}: application, configuration, consumables, safety controls and quotation preparation.`, 155),
    canonicalUrl: `${SITE_URL}/news/${slugify(title)}`,
    primaryKeyword: primary.title,
    secondaryKeywords: [category, scenario, "power tool buyer guide"],
    geoSummary: `Cowin Supply buyer guidance for evaluating ${primary.title} in global distributor and contractor workflows.`,
    faq,
    industry: "professional power tools",
    scenario,
    keyTakeaways: ["Define the task and material before selecting a configuration.", "Confirm voltage, plug, consumables and destination requirements.", "Treat the catalog as the start of a technical review, not a performance guarantee."]
  };
}
...
<h2>Task review for ${escapeHtml(primary.title)}</h2><p>${escapeHtml(productReviewGuidance(primary))}</p>
<h2>Configuration and documentation check</h2><p>Before a distributor or contractor treats a catalog item as suitable for a specific package of work, the technical conversation should capture the conditions that cannot be inferred from a headline or a general product category. This includes the material and finish condition, dimensional requirement, electrical supply at the point of work, expected shift pattern, access restrictions, dust or water controls, and the consumable or accessory format already used by the crew. Where a destination market requires product documentation, the request should also identify the applicable market and any local review process. Recording these details makes the discussion traceable and reduces the risk of treating a general workflow reference as a confirmed configuration. The final choice remains dependent on the product record, the intended application and the buyer's own safety process.</p>
...
function selectNewsCoverImage({ raw, primary, related, products, existingNews }) {
  ...
  for (const item of publishedNews) {
    const image = normalizeNewsImage(item.image);
    if (!image) continue;
    const usage = imageUsage.get(image) || { count: 0, lastUsedAt: 0 };
    usage.count += 1;
    usage.lastUsedAt = Math.max(usage.lastUsedAt, new Date(item.publishedAt || 0).getTime() || 0);
    imageUsage.set(image, usage);
  }

  // Do not repeat any of the last twelve News covers. Once the whole owned-media
  // catalogue has been used, choose the least recently used image instead.
  const recentlyUsed = new Set(publishedNews.slice(0, 12).map((item) => normalizeNewsImage(item.image)).filter(Boolean));
  const allProducts = uniqueNewsImageProducts(products);
  const relatedProducts = uniqueNewsImageProducts(related);
  const sameCategoryProducts = uniqueNewsImageProducts(products.filter((product) => product.category === primary.category));
  const preferredPools = [relatedProducts, sameCategoryProducts, allProducts];
  const pool = preferredPools.find((items) => items.some((item) => !recentlyUsed.has(normalizeNewsImage(item.image)))) || allProducts;
  const available = pool.filter((item) => !recentlyUsed.has(normalizeNewsImage(item.image)));
  const candidates = available.length ? available : pool;
  const product = selectLeastUsedNewsImage(candidates, imageUsage, raw) || primary;
  return { image: product.image, product };
}

function uniqueNewsImageProducts(products = []) {
  return products
    .filter((product) => product?.image)
    .filter((product, index, list) => list.findIndex((entry) => normalizeNewsImage(entry.image) === normalizeNewsImage(product.image)) === index);
}
...
function selectLeastUsedNewsImage(products, imageUsage, raw) {
  const ranked = [...products].sort((left, right) => {
    const leftUsage = imageUsage.get(normalizeNewsImage(left.image)) || { count: 0, lastUsedAt: 0 };
    const rightUsage = imageUsage.get(normalizeNewsImage(right.image)) || { count: 0, lastUsedAt: 0 };
    return leftUsage.count - rightUsage.count || leftUsage.lastUsedAt - rightUsage.lastUsedAt || String(left.title).localeCompare(String(right.title));
  });
  if (!ranked.length) return null;
  const best = imageUsage.get(normalizeNewsImage(ranked[0].image)) || { count: 0, lastUsedAt: 0 };
  const equallyEligible = ranked.filter((product) => {
    const usage = imageUsage.get(normalizeNewsImage(product.image)) || { count: 0, lastUsedAt: 0 };
    return usage.count === best.count && usage.lastUsedAt === best.lastUsedAt;
  });
  const offset = Number.parseInt(String(raw.eventFingerprint || raw.sourceFingerprint || "0").slice(0, 8), 16) || 0;
  return equallyEligible[offset % equallyEligible.length] || ranked[0];
}
...
function buildPublishedArticle(candidate, products) {
  ...
  const isOriginal = candidate.contentKind === "original_editorial";
  return {
    type: "news", slug, title: article.title, category: isOriginal ? "Buyer Briefing" : candidate.source?.publisher || "Industry News", image: article.image, summary: article.summary, status: "published", seoIndexable: true,
    ...
    generationModel: "deterministic-editorial-v3", generationPromptVersion: NEWS_POLICY_VERSION, relatedProducts: candidate.productSlugs.map((productSlug) => ({ productSlug })), originalFacts: candidate.sourceSummary,
...
function dateSeed(value) { return [...String(value)].reduce((total, character) => total + character.charCodeAt(0), 0); }
function canPublish(state, candidate, { allowRotationFallback = false } = {}) {
  if (candidate?.status !== "scheduled" || candidate.rejectionReasons?.length || shanghaiDay(state.lastSuccessfulPublishAt || "1970-01-01") === shanghaiDay()) return false;
  return allowRotationFallback || !candidate.availabilityReasons?.length;
}
...
function nextExecutionReason(state, candidates = []) {
  if (shanghaiDay(state.lastSuccessfulPublishAt || "1970-01-01") === shanghaiDay()) return "daily_publish_limit_reached";
  if (candidates.some((item) => item.status === "scheduled" && item.availabilityReasons?.length)) return "only_rotation_deferred_candidates";
  if (candidates.some((item) => item.status === "needs_review" || item.status === "rejected")) return "all_current_candidates_blocked_by_quality_gates";
  return "no_quality_checked_candidate";
}
async function getConfig() {
  ...
  return {
    ...
    publishIntervalHours: 24,
    version: NEWS_POLICY_VERSION,
    sourceWhitelist: normalizeConfiguredSources(["v4", NEWS_POLICY_VERSION].includes(saved.version) ? saved.sourceWhitelist : DEFAULT_SOURCES, DEFAULT_SOURCES),
    sourceBlacklist: normalizeSourceIds(saved.sourceBlacklist, [])
  };
}
async function migrateLegacyRules() {
  if (!hasPersistentStore() || (await getPersistentValue(MIGRATION_KEY))?.version === NEWS_POLICY_VERSION) return;
  ...
  const nextConfig = {
    ...
    updatedAt: new Date().toISOString(),
    migrationNote: "Reliable daily publishing: reassess legacy candidates, allow a verified seven-day source fallback, and use an original Cowin Supply buyer briefing only when external candidates remain blocked."
  };
  await Promise.all([
    setPersistentValue(CONFIG_KEY, nextConfig),
    setPersistentValue(MIGRATION_KEY, { version: NEWS_POLICY_VERSION, migratedAt: new Date().toISOString(), preservedPublishedNews: true, supersededRules: ["non-observable quality-gate stall", "72-hour hard cutoff without fallback"], note: "Daily News automation now guarantees an auditable publication attempt with frontend verification. Blog automation was not changed." })
  ]);
}
...
async function verifyPublishedNews(article) {
  let latest = { visible: false, checkedAt: new Date().toISOString(), reason: "title_not_visible_on_frontend" };
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    const cacheBust = `news_verification=${Date.now()}-${attempt}`;
    const urls = [`${SITE_URL}/news/${article.slug}?${cacheBust}`, `${SITE_URL}/news?${cacheBust}`];
    try {
      const responses = await Promise.all(urls.map((url) => fetch(url, {
        headers: { "user-agent": "CowinSupply-NewsVerifier/2.0", "cache-control": "no-cache" },
      ...
      latest = { visible: detailVisible && listVisible, checkedAt: new Date().toISOString(), attempt, detailStatus: responses[0].status, listStatus: responses[1].status, detailTitleVisible: detailVisible, listTitleVisible: listVisible, reason: detailVisible && listVisible ? "visible" : "title_not_visible_on_frontend" };
      if (latest.visible) return latest;
    } catch (error) {
      latest = { visible: false, checkedAt: new Date().toISOString(), attempt, reason: sanitizeError(error) };
    }
...
function productReviewGuidance(product) {
  const text = `${product.title} ${product.category}`.toLowerCase();
  if (/chaser|slot/.test(text)) return "For channel-cutting work, the review starts with the wall material, the intended route and the required channel dimensions. The work method should account for reinforcement risk, existing services, dust control, access along the route and the blade configuration that is approved for the material. A machine can be relevant to the task without determining the final depth, width or work sequence; those points need confirmation against the actual site and the current product specification.";
  if (/drill|core/.test(text)) return "For drilling work, the review should identify the base material, opening position, required diameter, access from each side of the structure and the intended debris or water-control method. The team should also confirm how the work area will be marked, how the machine will be supported, and whether the available electrical supply matches the final configuration. These checks keep the product discussion tied to the actual installation task rather than to a general construction reference.";
  if (/grind|polish/.test(text)) return "For grinding or surface-finishing work, buyers should describe the substrate, desired finish, working position and the abrasive or accessory system already used on site. Dust extraction, ventilation, edge access and expected duty cycle can materially affect the configuration discussion. The purpose of the review is to match a confirmed setup to the preparation task, not to infer a result from an external industry update.";
  if (/laser|measur/.test(text)) return "For measurement work, the review should identify the working range, lighting conditions, reference surfaces, required measurement modes and any handover tolerance that the crew must follow. The tool is one part of a controlled layout process that also depends on marking practice, access and verification by the responsible installer. These conditions should be agreed before a configuration is selected for a project.";
  if (/saw|cut/.test(text)) return "For cutting work, the review should identify the material profile, required cut geometry, available clearance, guarding approach and the consumable format appropriate to the task. The operator's work position, debris management and power supply should be considered alongside the catalog record. This helps procurement teams separate an application fit discussion from any unsupported claim about the external project described by the source.";
  return "For professional tool workflows, the review should identify the material, access conditions, power supply, consumable format and safety controls that apply to the actual task. Those details allow a catalog record to be checked against a real requirement without converting a general industry update into a product-performance claim.";
}
...
function sourceGroupConsecutiveLimitReached(source, existingNews, config) {
  ...
  const recentAutomated = existingNews
    .filter((item) => item.status === "published" && ["v4", NEWS_POLICY_VERSION].includes(item.automationVersion) && item.sourceGroup)
...
function describeNextNewsAction({ state, scheduled, latestPublish }) {
  if (shanghaiDay(state.lastSuccessfulPublishAt || "1970-01-01") === shanghaiDay()) return "Today already has a frontend-verified News publication.";
  if (scheduled.some((item) => !item.availabilityReasons?.length)) return "A quality-checked candidate is ready for the next publishing window.";
  if (scheduled.some((item) => item.availabilityReasons?.length)) return "Only rotation-deferred candidates remain; the next run may use one as a logged fallback.";
  if (latestPublish?.reason === "all_current_candidates_blocked_by_quality_gates") return "All current candidates were blocked by quality gates. The next run will fetch fresh source items.";
  return "No publishable candidate is available yet. The next run will refresh source health and the candidate pool.";
}
