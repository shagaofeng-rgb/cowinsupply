...
import { getEligibleCatalogSources, getNewsSourceCatalogSummary } from "@/lib/newsSourceCatalog";
...
async function runIngestUnlocked({ trigger, dryRun }) {
  ...
  const enabledSources = config.sourceWhitelist.filter((item) => item.enabled !== false && item.catalogVerified && !config.sourceBlacklist.includes(item.id));
...
async function getConfig() {
  ...
  return { ...DEFAULT_CONFIG, ...saved, publishIntervalHours: 24, sourceWhitelist: saved.version === "v4" ? (saved.sourceWhitelist || DEFAULT_SOURCES) : DEFAULT_SOURCES };
}
