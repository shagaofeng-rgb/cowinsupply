...
async function runUnlocked({ trigger, dryRun }) {
  ...
  const candidates = refreshCandidatesForDirectPublication(savedCandidates, { products, existingNews, config });
  ...
  if (!pendingCandidate && !isExecutionDue(state, config)) {
    return recordRun({ trigger, dryRun, status: "skipped", reason: "full_48_hour_cycle_not_due", startedAt });
  }
  ...
  if (eligible && !dryRun) {
    ...
    try {
      ...
      nextCandidates[publishIndex] = { ...eligible, status: "failed", failedAt: new Date().toISOString(), retryCount: Number(eligible.retryCount || 0) + 1, rejectionReasons: [sanitizeError(error)] };
...
function refreshCandidatesForDirectPublication(candidates, { products, existingNews, config }) {
  return candidates.map((candidate) => {
    if (candidate.status !== "needs_review" || !candidate.sourceUrl || !candidate.sourceTitle) return candidate;
    const raw = {
    ...
    if (rebuilt.status !== "scheduled") return candidate;
    return {
      ...candidate,
      ...rebuilt,
      id: candidate.id,
      discoveredAt: candidate.discoveredAt,
      directPublicationEnabledAt: new Date().toISOString()
    };
  });
}
...
function isRetryableCandidate(candidate, state, config) {
  return ["scheduled", "failed"].includes(candidate?.status) && Boolean(candidate?.article) && Date.now() - new Date(state.lastSuccessfulPublishAt || 0).getTime() >= config.publishIntervalHours * 3600000;
}
