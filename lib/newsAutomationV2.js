...
async function runPublicationUnlocked({ trigger, dryRun }) {
  ...
  const fallbackCandidate = !primaryCandidate
    ...
    : null;
  const eligible = primaryCandidate || fallbackCandidate;
  const selectionMode = fallbackCandidate ? "rotation_fallback" : "primary";
  ...
  if (!dryRun) await setValue(CANDIDATES_KEY, nextCandidates.slice(0, MAX_CANDIDATES));
  return recordRun({ phase: "publish", trigger, dryRun, status: publishResult.published ? "published" : eligible ? "failed" : "completed", startedAt, quality: { scheduled: nextCandidates.filter((item) => item.status === "scheduled").length, deferredByRotation: nextCandidates.filter((item) => item.status === "scheduled" && item.availabilityReasons?.length).length, rejected: nextCandidates.filter((item) => item.status === "rejected").length }, publishResult });
}
...
function prepareCandidate(raw, { products, existingNews, candidates, config }) {
  ...
  const availabilityReasons = reasons.filter((reason) => ROTATION_REASONS.has(reason));
  const blockingReasons = publicationBlockers(reasons.filter((reason) => !ROTATION_REASONS.has(reason)));
...
function prepareIngestCandidate(raw, { products, existingNews, candidates, config }) {
  ...
  const availabilityReasons = reasons.filter((reason) => ROTATION_REASONS.has(reason));
  const qualityReasons = reasons.filter((reason) => !ROTATION_REASONS.has(reason));
