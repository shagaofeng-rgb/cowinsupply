...
// Keep business notifications aligned with the public contact address.
export const notificationRecipient = companyProfile.email;
...
function inquiryEmailHtml(inquiry) {
  const rows = [
    ...
    ["Product model", inquiry.productModel],
    ["Country / region", inquiry.country],
    ["Buyer type", inquiry.buyerType],
    ["Estimated quantity", inquiry.estimatedQuantity],
    ["Required voltage / specification", inquiry.requiredSpecification],
    ["Source URL", inquiry.pageUrl],
...
export async function sendAdminInquiryEmail(inquiry) {
  ...
  return { sent: true, messageId: info.messageId, recipient: notificationRecipient };
}
