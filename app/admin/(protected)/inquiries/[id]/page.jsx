...
export default async function InquiryDetailPage({ params }) {
  ...
  if (!detail) notFound();
  const { inquiry, journey, summary, journeyAvailable, customer = {}, relatedInquiries = [] } = detail;
  ...
  return <>
    ...
    <section className="admin-card inquiry-customer-card"><small>Customer profile</small><h2>All records linked to this customer</h2><dl className="inquiry-customer-facts"><div><dt>Identity</dt><dd>{customer.identity || "Not identified"}</dd></div><div><dt>Linked inquiries</dt><dd>{relatedInquiries.length || 1}</dd></div><div><dt>Total visits</dt><dd>{customer.totalVisits || summary.totalVisits || journey.length || 0}</dd></div><div><dt>Unique pages</dt><dd>{customer.uniquePaths || summary.uniquePaths || 0}</dd></div></dl></section>
    ...
    <section className="admin-card journey-card"><small>Visitor journey</small><h2>All page-view paths for this customer</h2><p className="admin-muted">Records are linked only through the submitted email, phone, visitor ID or session ID. No browsing history is inferred.</p>{!journeyAvailable ? <div className="empty-line">This older inquiry does not have a visitor ID, so no reliable path can be shown.</div> : null}{journeyAvailable && !journey.length ? <div className="empty-line">The form contains a visitor ID, but there are no matching page-view events yet.</div> : null}{journey.length ? <ol className="journey-timeline">{journey.map((event) => <li key={event.id}><time>{formatTime(event.createdAt)}</time><div><strong>{event.path}</strong><p>{event.title || "No page title"}</p><span>{event.source || "Direct"} · {event.device || "Unknown device"} · {event.country || "Unknown country"}</span>{event.referrer ? <small>Referrer: {event.referrer}</small> : null}</div></li>)}</ol> : null}</section>
    <section className="admin-card inquiry-related-card"><small>Linked inquiries</small><h2>All form submissions from this customer</h2>{relatedInquiries.length ? <ol className="inquiry-related-list">{relatedInquiries.map((item) => <li key={item.id}><time>{formatTime(item.createdAt)}</time><div><strong>{item.product || "General inquiry"}</strong><p>{item.message || "No message provided."}</p><span>{item.status || "new"}</span></div></li>)}</ol> : <div className="empty-line">No linked form submissions were found.</div>}</section>
  </>;
}
